...
void cloud_cb(const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
{
    ...
    pass.setFilterLimits(-5.0, 10.0); // 앞뒤 거리 제한
    pass.filter(*cloud);

    pass.setFilterFieldName("y");
    pass.setFilterLimits(-5.0, 5.0); // 좌우 거리 제한
    pass.filter(*cloud);

    pass.setFilterFieldName("z");
    pass.setFilterLimits(-1.0, 1.0); // 상하 거리 제한
    pass.filter(*cloud);
    ...
    for (const auto& cluster : clusters)
    {
        ...
        if (width <= 6.0 && depth <= 6.0 && height >= 0.1 && height <= 2)
        {
            // 네 가지 점 계산
...
int main(int argc, char** argv)
{
    ...
    obstacle_pub = nh.advertise<std_msgs::Float32MultiArray>("obstacle_data", 1);
...
//     {
//         ROS_WARN("Filtered cloud has no points.");
//         return;
//     }
...
//     arma::Row<size_t> assignments;
//     bool success = dbscan.Cluster(data, assignments);

//     if (!success || assignments.n_elem == 0)
//     {
//         ROS_WARN("DBSCAN did not produce any clusters.");
//         return;
//     }
...
//     for (const auto& cluster : clusters)
//     {
//         if (cluster.second->empty()) continue; // 클러스터가 비어 있으면 건너뜀
