...
class NIA_SEGNet_module(pl.LightningModule):
    ...
    def __init__(self):
        super().__init__()
        self.validation_step_outputs = [] #insert
        self.fcn = fcn_resnet50(weights=True)
        in_channels = 2048
    ...
    def test_step(self, batch, batch_idx):
        ...
        for i, output in enumerate(out):
            final_out = torch.argmax(output, 0)  # 각 픽셀의 클래스 예측
            
            colored_output = np.zeros((final_out.shape[0], final_out.shape[1], 3), dtype=np.uint8)
            for label in torch.unique(final_out):  # 예측된 클래스들에 대해 색상을 할당
                mask = final_out == label
                colored_output[mask.cpu()] = self.get_label_color(label.item())

            img = x[i].cpu().permute((1, 2, 0)).numpy()
            img = img[:, :, ::-1]
            plt.imsave("input.png", img)
            plt.imsave("output.png", colored_output)
            plt.imsave("target.png", y[i].cpu().numpy(), vmin=0, vmax=3)
            input()
    ...
    def test_dataloader(self):
        ...
        train_loader = DataLoader(dataset, batch_size = self.batch_size, num_workers=12,shuffle=False)
        return train_loader
