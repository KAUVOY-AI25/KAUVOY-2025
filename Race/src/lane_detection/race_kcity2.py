...
class FrameProcessor:
    def __init__(self, model, output_topic, steering_angle_topic):
        ...
        self.mode5_start_time = None 
        self.mode10_activated = False 

    def set_mode(self, mode):
        # Mode10이 한 번 활성화되면 다시 Mode5로 전환되지 않도록
        if self.mode10_activated and mode == 5:
            rospy.loginfo("Mode10이 활성화된 이후로 Mode5로 돌아가지 않습니다.")
            return

        # Mode5로 처음 전환될 때만 시작 시간 기록
        if mode == 5 and self.mode != 5:
            self.mode5_start_time = time.time()  
            rospy.loginfo("Mode5로 변경되었습니다. 2초 후 Mode10으로 전환 예정.")
        elif mode != 5:
            # 다른 모드로 변경될 때는 시작 시간 초기화
            self.mode5_start_time = None
        self.mode = mode  # Mode 설정
    ...
    def process_frame(self, frame, frame_idx):
        if self.mode == 5 and self.mode5_start_time:
            if time.time() - self.mode5_start_time >= 2:
                self.set_mode(10)  # Mode10으로 전환
                self.mode10_activated = True  # Mode10 활성화
                rospy.loginfo("Mode5에서 2초가 지나 Mode10으로 전환되었습니다.")
    ...
    def calculate_roi_corners(self, width, height):
        if self.mode == 3 or self.mode == 4:
            ...
            return np.array([[
                ...
                (width * 0.85, height * 0.95),  # 우측 하단
                (width * 0.85, height * 0.15),        # 우측 상단
...
def main():
    ...
    image_topic = 'camera1/usb_cam1/image_raw'
    
    rospy.Subscriber(image_topic, Image, frame_callback, frame_processor)  # 프레임 콜백 함수 설정
   
    # 모드를 수신하기 위한 구독
    mode_topic = '/mode_topic' # FROM) mode_kcity.py
