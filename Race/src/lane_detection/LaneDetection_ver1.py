...
# Dependency: None (파이썬 모듈은 다 불러와야 함)
# Description: 차선 인식 -> 왼쪽,오른쪽 차선 분리 -> visualize
# ================================================================================
...
class FrameProcessor:
    def __init__(self, model, output_topic):
        ...
        # !# 차선만 보도록 윗부분 제거한 사다리꼴로 설정
        self.roi_top_left_ratio = (0.15, 0.6)
        self.roi_top_right_ratio = (0.85, 0.6)
    ...
    def update_lane_points(self, output_predictions):
        ...
        # KDTree 생성 전 데이터 확인 및 초기화
        if self.left_lane_points is None or len(self.left_lane_points) == 0:
            self.left_lane_points = np.empty((0, 2))  # 빈 2차원 배열로 초기화
        if self.right_lane_points is None or len(self.right_lane_points) == 0:
            self.right_lane_points = np.empty((0, 2))  # 빈 2차원 배열로 초기화

        left_tree = KDTree(self.left_lane_points) if len(self.left_lane_points) > 0 else None
        right_tree = KDTree(self.right_lane_points) if len(self.right_lane_points) > 0 else None
